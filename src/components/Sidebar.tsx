...
import { 
  ...
  CheckSquare, Megaphone, DollarSign, Calendar, Library, BookMarked, QrCode, LogOut
} from 'lucide-react';
...
export default function Sidebar({ currentUser, activePage, onPageChange, isOpen, onClose, onOpenShareModal, onLogout }: SidebarProps) {
    ...
    <nav className={`sidebar fixed top-[60px] bottom-0 left-0 w-[240px] bg-slate-50 border-r border-slate-200 p-4 overflow-y-auto flex flex-col gap-1.5 z-40 theme-bg-surface theme-border transition-transform duration-300 ease-in-out ${
      ...
      {onOpenShareModal && (
        <button
          ...
          title="Compartir enlace con estudiantes y ver código QR"
        >
      ...
      {/* BOTÓN CERRAR SESIÓN / CAMBIAR USUARIO */}
      {onLogout && (
        <button
          type="button"
          onClick={onLogout}
          className="mt-2 flex items-center justify-center gap-2 p-2.5 bg-rose-50/80 hover:bg-rose-100 text-rose-700 rounded-xl text-[11px] font-bold tracking-tight transition cursor-pointer select-none border border-rose-200 shrink-0 shadow-xs"
