...
export default function ExamBuilder({ 
  ...
  const handleAddField = () => {
    ...
    setQuestions(prev => {
      ...
      latestDataRef.current.questions = next;
      return next;
    });
    setSaveStatus('saving');
  };
  ...
  const handleUpdateQText = (id: string, text: string) => {
    notifyUserEdit();
    setQuestions(prev => {
      const next = prev.map(q => q.id === id ? { ...q, texto: text } : q);
      latestDataRef.current.questions = next;
      return next;
    });
  };
  ...
  const handleUpdateQPts = (id: string, pts: number) => {
    notifyUserEdit();
    setQuestions(prev => {
      const next = prev.map(q => q.id === id ? { ...q, puntos: pts } : q);
      latestDataRef.current.questions = next;
      return next;
    });
  };
  ...
  const handleMoveQuestion = (index: number, dir: -1 | 1) => {
    ...
    setQuestions(prev => {
      ...
      latestDataRef.current.questions = nextQList;
      return nextQList;
    });
    setSaveStatus('saving');
  };
  ...
  const handleBack = async () => {
    ...
    try {
      ...
      console.warn('Auto-save on back notice:', e);
    }
    onBack();
  };
