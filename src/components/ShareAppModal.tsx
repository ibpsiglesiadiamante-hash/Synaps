...
// Helper to obtain the active, fully working live URL of the application
export function getActiveAppUrl(): string {
  ...
  return 'https://ais-pre-4lyu4xtxpsuhsxd6l7byl6-657422271043.us-west2.run.app';
}

export const FIREBASE_HOSTING_URL = 'https://synapsis-edu.web.app';
export const OFFICIAL_PORTAL_URL = FIREBASE_HOSTING_URL;
...
export default function ShareAppModal({ 
  ...
  const activeUrl = getActiveAppUrl();
  const [selectedUrlType, setSelectedUrlType] = useState<'active' | 'firebase'>('active');
  const targetUrl = selectedUrlType === 'active' ? activeUrl : FIREBASE_HOSTING_URL;
  ...
  const handleDownloadQr = () => {
    ...
    link.href = qrDataUrl;
    link.download = `codigo-qr-synapsis-${selectedUrlType}.png`;
    link.click();
  };
        ...
        <body>
          <div class="container">
            ...
            <div class="instrucciones">
              ...
              3. O ingresa directamente desde cualquier navegador a: ${targetUrl}
            </div>
          </div>
          <script>
  ...
  const handleShareWhatsApp = () => {
    const text = encodeURIComponent(
      ...
      `Acceso para evaluaciones, notas, parciales y tareas académicas.\n\n` +
      `🔗 *Enlace directo:* ${targetUrl}\n\n` +
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-base tracking-tight text-white">
                Compartir Portal con Estudiantes
              </h3>
              <p className="text-[11px] text-emerald-300 font-medium font-mono">
                {selectedUrlType === 'active' ? 'Enlace Directo Activo (En Vivo)' : 'synapsis-edu.web.app'}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-5 max-h-[80vh] overflow-y-auto" ref={printAreaRef}>
          ...
          <div className="bg-slate-100 p-1.5 rounded-2xl flex gap-1.5 border border-slate-200">
            <button
              type="button"
              onClick={() => setSelectedUrlType('active')}
              className={`flex-1 py-2 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer ${
                selectedUrlType === 'active'
                  ? 'bg-white text-indigo-900 shadow-sm border border-slate-200'
                  : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              <span>Enlace Directo Activo</span>
              <span className="bg-emerald-100 text-emerald-800 text-[10px] px-1.5 py-0.5 rounded font-bold">100% Funcional</span>
            </button>
            <button
              type="button"
              onClick={() => setSelectedUrlType('firebase')}
              className={`flex-1 py-2 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer ${
                selectedUrlType === 'firebase'
                  ? 'bg-white text-indigo-900 shadow-sm border border-slate-200'
                  : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              <span>synapsis-edu.web.app</span>
              <span className="bg-amber-100 text-amber-800 text-[10px] px-1.5 py-0.5 rounded font-bold">Hosting</span>
            </button>
          </div>

          {/* Aviso especial si seleccionó Firebase Hosting o si experimentó Site Not Found */}
          {selectedUrlType === 'firebase' ? (
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <p className="font-bold text-amber-950">
                    ¿Por qué sale "Site Not Found" en synapsis-edu.web.app?
                  </p>
                  <p className="text-[11px] text-amber-800 mt-1 leading-relaxed">
                    Firebase Hosting muestra <strong>"Site Not Found"</strong> porque este subdominio requiere que ejecutes el archivo <code className="bg-amber-100 px-1 py-0.5 rounded font-mono font-bold">desplegar.bat</code> desde tu PC con Firebase CLI instalado.
                  </p>
                </div>
              </div>
              <div className="pt-1 flex items-center justify-between border-t border-amber-200">
                <span className="text-[11px] font-semibold text-amber-800">
                  ¿Quieres compartir ahora mismo sin este error?
                </span>
                <button
                  type="button"
                  onClick={() => setSelectedUrlType('active')}
                  className="px-3 py-1 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-lg text-[11px] transition-colors cursor-pointer"
                >
                  Usar Enlace Directo Activo
                </button>
              </div>
            </div>
          ) : (
            <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-3.5 flex items-start gap-2.5 text-xs text-emerald-950">
              ...
              <div>
                <p className="font-bold text-emerald-900">
                  Enlace 100% Funcional en Vivo para Estudiantes
                </p>
                <p className="text-[11px] text-emerald-800 mt-0.5">
                  Este es el enlace donde tu aplicación está ejecutándose actualmente. Tus estudiantes pueden abrirlo desde cualquier celular o computador sin encontrarse con pantallas de error.
                </p>
              </div>
            </div>
          )}

          {/* Active Verified URL Section */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-700 flex items-center justify-between">
              <span className="flex items-center gap-1.5 text-indigo-950 font-bold">
                <Globe className="w-3.5 h-3.5 text-emerald-600" />
                <span>Enlace a Compartir:</span>
              </span>
              <span className={`text-[11px] font-bold px-2 py-0.5 rounded-md border ${
                ...
                {selectedUrlType === 'active' ? '✓ Activo y Seguro' : 'Requiere Deploy'}
              </span>
            </label>
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-white border-2 border-indigo-400 rounded-xl p-3 flex items-center justify-between overflow-hidden shadow-xs">
                <span className="font-mono font-bold text-indigo-900 text-xs sm:text-sm truncate">
                  {targetUrl}
                </span>
                <a
                  href={targetUrl}
                  ...
                  className="text-indigo-600 hover:text-indigo-800 p-1 ml-2 transition-colors shrink-0"
          ...
          <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-5 flex flex-col sm:flex-row items-center gap-5">
            ...
            <div className="flex-1 space-y-2 text-center sm:text-left">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                Apunta la cámara para ingresar inmediatamente a <span className="font-semibold text-slate-700 break-all">{targetUrl}</span>.
              </p>
          ...
          <div>
            ...
            >
              ...
              <span>Compartir Enlace por WhatsApp</span>
            </button>
          </div>

          {/* Firebase Connection & Database Sync Banner */}
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                    <span>Base de Datos Cloud Firestore</span>
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider bg-emerald-100 text-emerald-800 px-2.5 py-0.5 rounded-md border border-emerald-200">
                En Línea
              </span>
            </div>
              ...
              >
                ...
                <span>
                  {isCurrentSyncing 
                    ...
                    : localSyncState === 'success'
                      ? `¡Datos guardados en la nube! ${lastSyncCount ? `(${lastSyncCount} registros)` : ''}`
                      : 'Sincronizar y Guardar Base de Datos en la Nube'
                  }
                </span>
              </button>
            )}
          ...
          {/* Guía desplegable de Firebase Hosting */}
          <div className="border border-slate-200 rounded-2xl overflow-hidden">
            ...
            >
              <span>¿Cómo publicar en synapsis-edu.web.app si lo necesito?</span>
              <span className="text-slate-400 font-mono text-sm">{showDeployHelp ? '▲' : '▼'}</span>
            </button>
            {showDeployHelp && (
              <div className="p-4 bg-white space-y-2 text-xs text-slate-600 border-t border-slate-200 leading-relaxed">
                <p>
                  Si deseas que la dirección sea exactamente <strong className="text-slate-800">synapsis-edu.web.app</strong>, sigue estos sencillos pasos en tu computador:
                </p>
                <ol className="list-decimal list-inside space-y-1 pl-1 text-[11px] text-slate-700">
                  <li>Descarga el código del proyecto como archivo ZIP desde el menú superior de AI Studio.</li>
                  <li>Descomprime la carpeta en tu computadora.</li>
                  <li>Haz doble clic en el archivo <code className="bg-slate-100 px-1 py-0.5 rounded font-mono font-bold text-indigo-700">desplegar.bat</code>.</li>
                  <li>El asistente compilará y subirá los archivos a Firebase Hosting automáticamente. Una vez termine, ¡el error "Site Not Found" desaparecerá!</li>
                </ol>
                <p className="text-[11px] text-emerald-700 font-medium bg-emerald-50 p-2 rounded-xl border border-emerald-200">
                  💡 <strong>Recuerda:</strong> No es obligatorio hacerlo ahora. Tus estudiantes ya pueden usar la plataforma sin problemas mediante el <strong>Enlace Directo Activo</strong>.
                </p>
              </div>
            )}
          </div>
