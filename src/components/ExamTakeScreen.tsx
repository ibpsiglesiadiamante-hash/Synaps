...
export default function ExamTakeScreen({ 
  ...
  const processSubmitPayload = () => {
    ...
    // Immediately persist and register submission so records are never lost
    clearPersistedData();
    try {
      onSubmit(submission);
    } catch (e) {
      console.warn('Error invoking onSubmit in ExamTakeScreen:', e);
    }
    try {
      saveDocToFirestore('submissions', submission).catch((err) => {
        console.warn('Direct Firestore save of submission in ExamTakeScreen:', err);
      });
    } catch (err) {
      console.warn('Firestore submission write dispatch notice:', err);
    }
    ...
    if (exam.mostrarNota) {
      ...
      toast('Examen enviado y calificado exitosamente ✓', 'success');
      onExit();
    }
  };
  ...
  const handleCloseResultsModal = () => {
    ...
    toast('Examen completado y registrado en tu historial ✓', 'success');
    onExit();
  };
